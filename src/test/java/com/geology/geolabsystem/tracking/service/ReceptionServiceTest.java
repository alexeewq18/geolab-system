...
class ReceptionServiceTest {
    ...
    void registerShipments_ExistingOrder_Success() {
        ...
        // заказ уже найден — новый не создаётся
    ...
    void registerShipments_NewOrderCreated() {
        ...
        // новый заказ должен быть сохранён со статусом CREATED и amount = 0
    ...
    void registerShipments_DuplicateOrderDetails_SingleBalanceCall() {
        ...
        // одинаковые (orderName, description, geologistName) — пересчёт один раз
        verify(orderControlService, times(1)).calculateBalance(
                "0924Д", "Мосты", "Иванов"
        );
    }
    ...
    void getAllShipments_ReturnsList() {
        ...
        when(shipmentsRepository.findAll()).thenReturn(List.of(entity));
        when(shipmentsMapper.toResponseDto(any(ShipmentEntity.class))).thenReturn(responseDto);

        List<ShipmentResponseDto> result = receptionService.getAllShipments();
    ...
    void getAllShipments_EmptyList() {
        when(shipmentsRepository.findAll()).thenReturn(Collections.emptyList());
