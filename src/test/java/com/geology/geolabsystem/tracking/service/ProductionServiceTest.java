...
public class ProductionServiceTest {
    ...
    void getAllDailyWorks_ReturnsList() {
        ...
        DailyWorksEntity entity = new DailyWorksEntity();
        Page<DailyWorksEntity> page = new PageImpl<>(List.of(entity));

        when(dailyWorksRepository.findAll(pageable)).thenReturn(page);
        when(mapper.toResponseDto(any(DailyWorksEntity.class))).thenReturn(responseDto);

        Page<DailyWorksResponseDto> result = productionService.getAllDailyWorks(pageable);

        assertEquals(1, result.getTotalElements());
        verify(dailyWorksRepository, times(1)).findAll(pageable);
    }
    ...
    void getAllDailyWorks_EmptyList() {
        Pageable pageable = PageRequest.of(0, 10);
        Page<DailyWorksEntity> emptyPage = new PageImpl<>(List.of());

        when(dailyWorksRepository.findAll(pageable)).thenReturn(emptyPage);
