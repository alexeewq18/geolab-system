...
class OrderControlServiceTest {
    ...
    void getAllOrders_ReturnsList() {
        when(labOrdersRepository.findAll()).thenReturn(List.of(testOrder));
        when(mapper.toResponseDto(any(LabOrderEntity.class))).thenReturn(responseDto);

        List<LabOrderResponseDto> result = orderControlService.getAllOrders();

        assertEquals(1, result.size());
        verify(labOrdersRepository, times(1)).findAll();
    ...
    void getAllOrders_EmptyList() {
        when(labOrdersRepository.findAll()).thenReturn(Collections.emptyList());
